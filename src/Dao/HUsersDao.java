...
public class HUsersDao {
    ...
    *
    * */
    public List login(HUsersEntity hUsersEntity) {
        String userName = hUsersEntity.getTelephoneNumber();
        String passWord = hUsersEntity.getPassword();
        DaoFactory<HUsersEntity> daoFactory = new DaoFactory<>();
        List<HUsersEntity>list = daoFactory.cursor(hUsersEntity, "select * from h_users as u where u.telephone_number='" + userName + "'and u.password='" + passWord + "'", HUsersEntity.class);
...
//        if (list.size() != 0) return true;
    ...
    * */
    public List getSpecialProfile() {
        DaoFactory<HUsersEntity> daoFactory = new DaoFactory<>();
        HUsersEntity hUsersEntity = new HUsersEntity();
        List<HUsersEntity> list = daoFactory.cursor(hUsersEntity, "select * from h_users limit 10", HUsersEntity.class);
        return list;
    }
